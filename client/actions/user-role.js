...
export const fetchRoles = () => {
  ...
  return (dispatch) => {
    ...
    return fetch(`/graphql?${queryQl}`)
      .then(res => res.json())
      .then(json => dispatch(receiveRoles(json.data.roles.roles))
      );
  };
};
...
export const fetchUsers = (rid) => {
  const queryQl = rid ?
   `query={users(role:${rid}){users{_id,loginname,useralias,roles{_id,name}}}}` :
   'query={users{users{_id,loginname,useralias,roles{_id,name}}}}';
  return (dispatch) => {
    dispatch(requestUsers(rid));
    return fetch(`/graphql?${queryQl}`)
      .then(res => res.json())
      .then((json) => {
        dispatch(receiveUsers(json.data.users.users));
      });
  };
};
...
export const updateUserRoles = (uid, rids) => {
  ...
  const qStr = `[${ridsStr}]`;
  console.log(qStr);
  const mutationQl = `mutation{updateUserRoles(input:{id:"${uid}",value:${qStr}}){_id,loginname,roles{_id,name}}}`;
